...
public class UnitResource {
    ...
    public ResponseEntity<Unit> createUnit(@RequestBody Unit unit){
        return ResponseEntity.created(URI.create("/units/" + unit.getId())).body(unitService.createUnit(unit));
    }

    @GetMapping
    public ResponseEntity<Page<Unit>> getUnits(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size){
